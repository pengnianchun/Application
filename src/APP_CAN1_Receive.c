...
unsigned int fADV[2]; //前模块电压值
unsigned int mADV[2]; //中（顶）模块电压值
unsigned int rADV[2]; //后模块电压值
...
void __attribute__((__interrupt__, auto_psv)) _C1Interrupt(void) {
    ...
    if (C1RX0CONbits.RXFUL) { //数据有效
        ...
        switch (message.id) {
            ...
            case 0x671:
                Fcan_count = 0;
                fADR[0] = ((message.data[0]+((unsigned int) message.data[1] << 8)) / 10);
                fADR[1] = ((message.data[2]+((unsigned int) message.data[3] << 8)) / 10);
                fADV[0] = message.data[4]+((unsigned int) message.data[5] << 8);
                fADV[1] = message.data[6]+((unsigned int) message.data[7] << 8);
                break;
            case 0x672:
                Mcan_count = 0;
                mADR[0] = ((message.data[0]+((unsigned int) message.data[1] << 8)) / 10);
                mADR[1] = ((message.data[2]+((unsigned int) message.data[3] << 8)) / 10);
                mADV[0] = message.data[4]+((unsigned int) message.data[5] << 8);
                mADV[1] = message.data[6]+((unsigned int) message.data[7] << 8);
                break;
            case 0x673:
                Rcan_count = 0;
                rADR[0] = ((message.data[0]+((unsigned int) message.data[1] << 8)) / 10);
                rADR[1] = ((message.data[2]+((unsigned int) message.data[3] << 8)) / 10);
